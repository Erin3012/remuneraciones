...
if(!$liquidations||!is_file($liquidations)||!$book||!is_file($book)){fwrite(STDERR,"Uso: php tools/validate_external.php <LIQUIDACIONES JULIO.pdf> <libro de rem. junio 2026.pdf> [--no-db]\n");exit(2);}
...
$bookExpected=[];if(isset($bookTotals[0])){$bookExpected=['total_imponible'=>$bookTotals[0][5]??null,'total_no_imponible'=>$bookTotals[0][8]??null,'total_haberes'=>$bookTotals[0][9]??null];}if(isset($bookTotals[1])){$bookExpected['total_descuentos_legales']=$bookTotals[1][6]??null;$bookExpected['total_descuentos']=$bookTotals[1][8]??null;$bookExpected['liquido']=$bookTotals[1][9]??null;}$bookActual=[];if($company){$s=$pdo->prepare('SELECT p.calculation_json FROM payslips p JOIN payroll_periods pp ON pp.id=p.period_id WHERE pp.company_id=? AND pp.period=?');$s->execute([$company['id'],'2026-06']);foreach($s as $r){$calc=json_decode((string)$r['calculation_json'],true)?:[];foreach(['total_imponible'=>'taxable','total_no_imponible'=>'nonTax','total_haberes'=>'haberes','liquido'=>'net'] as $dest=>$src)$bookActual[$dest]=($bookActual[$dest]??0)+(int)($calc[$src]??0);$bookActual['total_descuentos']=($bookActual['total_descuentos']??0)+(int)($calc['discounts']??0);$bookActual['total_descuentos_legales']=($bookActual['total_descuentos_legales']??0)+(int)($calc['afp']??0)+(int)($calc['health']??0)+(int)($calc['iusc']??0)+(int)($calc['scWorker']??0);}}
$out=__DIR__.'/external_validation_'.date('Ymd_His').'.csv';$fp=fopen($out,'wb');foreach($csv as $line)fputcsv($fp,$line,';');fclose($fp);echo "Validación externa\nEmpresa: ".($company['name']??'No encontrada')." ($companyRut)\nLiquidaciones: ".count($external)." trabajadores externos\nComparaciones: $compared | Coinciden: $matches | Diferencias: $differences | No encontrados: $missing\nTotales generales libro junio detectados: ".count($bookTotals)."\n";foreach($bookExpected as $key=>$value){$actualValue=$bookActual[$key]??null;$diff=$actualValue===null||$value===null?'n/d':(string)($actualValue-$value);echo "Libro junio $key: externo=".($value??'n/d')." aplicación=".($actualValue??'n/d')." diferencia=$diff\n";}echo "Modo: ".($noDb?'extracción sin base de datos':'comparación con base de datos')."\nInforme: $out\n";
